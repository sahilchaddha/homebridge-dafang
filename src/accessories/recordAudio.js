...
const RecordAudioSwitch = class extends Accessory {
  ...
  updateState(isOn) {
    if (isOn !== this.isOn) {
      this.log('Setting Record Audio Value to ' + isOn)
    }
    this.isOn = isOn
    const res = this.isOn
    this.services[0]
  ...
  getModelName() {
    return 'Record Audio Switch'
  }

  getSerialNumber() {
    return '00-001-RecordAudioSwitch'
  }
}

module.exports = RecordAudioSwitch
